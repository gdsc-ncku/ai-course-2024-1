...
def generate_messages(query: str, contexts: list[str], memory: list) -> list:
    return [
        {"role": "system", "content": generate_system_prompt(contexts)},
        *memory,
        {"role": "user", "content": query},
    ]

def inference(query: str, context: list[str], memory: list) -> str:
    system_prompt = generate_system_prompt(context)
    response = openai.chat.completions.create(
        model="gpt-4o-mini",
        messages=generate_messages(query, context, memory)
    )
...
if __name__ == "__main__":
    ...
    while input_prompt.lower() != "bye":
        ...
        # inference
        response = inference(input_prompt, contexts, memory)
